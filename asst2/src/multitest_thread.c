...
int search(int arr[], int arrLen, int target, int maxSize) {
    ...
    for (i = 0; i < numThreads; i++) {
        ...
        // Make the arguments for the thread since it can only take in a void*. 
        // We need to pass in three arguments so lets put it in a struct to make it simple.         
        ...
        pthread_create(threads + i, NULL, sequentialSearch, args);
    }
    ...
    *(args -> resultIdx) = -1;
    for (i = 0; i < args -> arrLen; i++)
         if (args -> arr[i] == args -> target) {
            // Return the int as a pointer. 
            // Since it's bad practice to return the address of a local variable, 
            // lets use another part of the struct to return the index.
            *(args -> resultIdx) = i;
            break;
        }
