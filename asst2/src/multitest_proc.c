...
int search(int arr[], int arrLen, int target) {
	...
	for (i = 0; i < numProcs; i++) {
		...
		if (pid > 0) {
			...
			// This loop is for finding which pid corresponds to which thread. 
            // The number of the pid in the index determines where its going to start looking in the array.
		   	for (i = 0; i < numProcs; i++) {
	...
	// Wait on the children to finish
	int status, index = -1;
	for (i = 0; i < numProcs; i++) { 
		if (index == -1) { // If we haven't found the target yet, wait.
			...
			if (WIFEXITED(status)) { // Check if the process ended normally.
                ...
                // The exit status will return a number between 0 and 249. Add it to where the child started its loop.
